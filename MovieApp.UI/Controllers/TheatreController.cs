...
namespace MovieApp.UI.Controllers
{
    public class TheatreController : Controller
    {
        ...
        public async Task<IActionResult> DeleteTheatreById(int theatreId)
        {
            using (HttpClient client = new HttpClient())
            {
                string endpoint = _configuration["WebApiURL"] + "Theatre/FindTheatreById?TheatreId=" + theatreId;
                using (var response = await client.GetAsync(endpoint))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                    {
        ...
        {
            string URL = _configuration["WebApiURL"] + "Theatre/FindTheatreById?TheatreId=" + theatreId;
            using (HttpClient client = new HttpClient())
            {
